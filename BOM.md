# Bill of Materials (BOM) - Fire-Fighting Robot Car

## Complete Component List with Estimated Costs (India)

### Core Electronics

| S.No | Component | Specification | Quantity | Approx. Cost (INR) | Notes |
|------|-----------|---------------|----------|-------------------|-------|
| 1 | Arduino Uno | ATmega328P, USB cable included | 1 | ₹400 - ₹600 | Original or compatible |
| 2 | L298N Motor Driver | Dual H-Bridge, 2A per channel | 1 | ₹150 - ₹250 | Includes heat sink |
| 3 | IR Flame Sensor | KY-026 or equivalent | 3 | ₹150 - ₹300 | ₹50-100 each |
| 4 | DC Gear Motors | 6V, 100-200 RPM with wheel | 2 | ₹200 - ₹400 | ₹100-200 each |
| 5 | Ultrasonic Sensor | HC-SR04 | 1 | ₹50 - ₹100 | Distance detection |
| 6 | Relay Module | 5V, Single Channel | 1 | ₹40 - ₹80 | For pump control |
| 7 | Water Pump | Mini submersible, 3-6V DC | 1 | ₹100 - ₹200 | ~2L/min flow |
| 8 | Battery | Li-ion 7.4V 2200mAh or 11.1V | 1 | ₹400 - ₹800 | 2S or 3S pack |
| 9 | Battery Charger | Compatible with Li-ion | 1 | ₹150 - ₹300 | Balance charger |
| 10 | Power Switch | SPST, 3A rated | 1 | ₹20 - ₹40 | On/Off control |

**Subtotal (Core Electronics): ₹1,660 - ₹3,070**

### Mechanical Components

| S.No | Component | Specification | Quantity | Approx. Cost (INR) | Notes |
|------|-----------|---------------|----------|-------------------|-------|
| 11 | Robot Chassis | Acrylic or metal, 2-layer | 1 | ₹200 - ₹400 | With mounting holes |
| 12 | Wheels | 65-70mm diameter | 2 | Included with motors | Usually with motors |
| 13 | Castor Wheel | Ball bearing, 360° rotation | 1 | ₹30 - ₹60 | Front/rear support |
| 14 | Water Tank | Plastic bottle or container | 1 | ₹0 - ₹50 | Reuse bottle or buy |
| 15 | Plastic Tubing | 6mm ID, flexible | 1m | ₹20 - ₹50 | For water delivery |
| 16 | Pipe/Nozzle | 6mm connector or straw | 1 | ₹10 - ₹30 | Water outlet |
| 17 | Cable Ties | Various sizes, pack of 100 | 1 pack | ₹50 - ₹100 | Wire management |
| 18 | Mounting Screws | M3 screws, nuts, spacers | Set | ₹50 - ₹100 | Hardware kit |
| 19 | Double-sided Tape | Strong adhesive | 1 roll | ₹30 - ₹60 | Component mounting |

**Subtotal (Mechanical): ₹390 - ₹850**

### Wiring & Connectors

| S.No | Component | Specification | Quantity | Approx. Cost (INR) | Notes |
|------|-----------|---------------|----------|-------------------|-------|
| 20 | Jumper Wires | Male-Male, Female-Female, Male-Female | 40 pcs set | ₹50 - ₹150 | Assorted pack |
| 21 | Single Core Wire | 22 AWG, various colors | 5m | ₹30 - ₹80 | For permanent connections |
| 22 | Heat Shrink Tubes | Assorted sizes | 1 pack | ₹40 - ₹80 | Insulation |
| 23 | Breadboard | 400 tie points (optional) | 1 | ₹50 - ₹100 | For prototyping |
| 24 | Connectors | JST/Bullet connectors | Set | ₹50 - ₹100 | Battery & motor connections |

**Subtotal (Wiring): ₹220 - ₹510**

### Optional Components (Upgrades)

| S.No | Component | Specification | Quantity | Approx. Cost (INR) | Notes |
|------|-----------|---------------|----------|-------------------|-------|
| 25 | Bluetooth Module | HC-05 or HC-06 | 1 | ₹200 - ₹350 | Remote control |
| 26 | Servo Motor | SG90, 9g micro servo | 1 | ₹100 - ₹200 | Directional spray |
| 27 | Buzzer | 5V active buzzer | 1 | ₹20 - ₹40 | Alert system |
| 28 | LEDs | 5mm, various colors | 5 | ₹10 - ₹30 | Status indicators |
| 29 | Temperature Sensor | DHT11 or DS18B20 | 1 | ₹50 - ₹150 | Heat monitoring |
| 30 | Camera Module | OV7670 or ESP32-CAM | 1 | ₹200 - ₹500 | Visual feedback |
| 31 | LCD Display | 16x2 I2C display | 1 | ₹150 - ₹300 | Status display |
| 32 | Battery Holder | For alternative power | 1 | ₹30 - ₹80 | If not using Li-ion |

**Subtotal (Optional): ₹760 - ₹1,650** (if all added)

### Tools & Consumables (One-time investment)

| S.No | Item | Specification | Quantity | Approx. Cost (INR) | Notes |
|------|------|---------------|----------|-------------------|-------|
| 33 | Soldering Iron | 25-40W, temperature control | 1 | ₹200 - ₹500 | For permanent connections |
| 34 | Solder Wire | Lead-free or 60/40 | 1 roll | ₹50 - ₹150 | Electronics grade |
| 35 | Wire Stripper | Multi-gauge | 1 | ₹100 - ₹300 | Wire preparation |
| 36 | Screwdriver Set | Phillips & flathead | 1 set | ₹150 - ₹400 | Assembly |
| 37 | Multimeter | Digital, voltage/current/resistance | 1 | ₹200 - ₹600 | Testing & debugging |
| 38 | Hot Glue Gun | With glue sticks | 1 | ₹100 - ₹300 | Securing components |
| 39 | Cutting Pliers | For wire cutting | 1 | ₹80 - ₹200 | Wire work |
| 40 | Helping Hands | Soldering assistant | 1 | ₹150 - ₹400 | Soldering aid |

**Subtotal (Tools): ₹1,030 - ₹2,850** (one-time)

## Cost Summary

| Category | Minimum Cost (INR) | Maximum Cost (INR) |
|----------|-------------------|-------------------|
| Core Electronics | ₹1,660 | ₹3,070 |
| Mechanical Components | ₹390 | ₹850 |
| Wiring & Connectors | ₹220 | ₹510 |
| **Basic Robot Total** | **₹2,270** | **₹4,430** |
| Optional Upgrades | ₹760 | ₹1,650 |
| Tools (one-time) | ₹1,030 | ₹2,850 |
| **Complete Setup** | **₹4,060** | **₹8,930** |

### Budget Options

#### 🟢 Minimum Budget Build (~₹2,500)
- Use Arduino clone (₹400)
- 2 flame sensors instead of 3 (₹200)
- Basic chassis and components
- Skip optional ultrasonic sensor initially
- Reuse household items (bottle, wires)

#### 🟡 Recommended Build (~₹3,500)
- Quality Arduino Uno
- All 3 flame sensors
- Good motor driver and motors
- Proper battery and charger
- Basic tools if not available

#### 🔴 Premium Build (~₹6,000+)
- Original Arduino
- All optional features (Bluetooth, camera, servo)
- Professional tools
- High-capacity battery
- Premium chassis and components

## Where to Buy (India)

### Online Platforms
1. **Robu.in** - Robotics components, good variety
2. **Amazon India** - Wide selection, fast delivery
3. **Flipkart** - Competitive pricing
4. **ThinkRobotics.in** - Robotics kits and components
5. **RobotShop.in** - Specialized robotics store
6. **IndiaMART** - Bulk purchases, wholesale
7. **ElectronicSpices.com** - Electronics components

### Offline Stores (Major Cities)
1. **Delhi** - Lajpat Rai Market, Nehru Place
2. **Mumbai** - Lamington Road
3. **Bangalore** - SP Road
4. **Chennai** - Ritchie Street
5. **Kolkata** - Chandni Chowk
6. **Hyderabad** - Secunderabad Market

### Local Options
- College/University electronics shops
- Robotics clubs and maker spaces
- Local electronics markets
- Wholesale dealers for bulk orders

## Cost-Saving Tips

### 💡 Budget Optimization
1. **Buy in Groups**: Share costs with classmates
2. **Reuse Components**: Salvage from old projects
3. **Local Markets**: Often cheaper than online
4. **Arduino Clones**: Work well, cost 50-70% less
5. **Wholesale**: Buy sensors in bulk for discounts
6. **DIY Chassis**: Use cardboard or recycled plastic
7. **Borrow Tools**: Use college lab tools
8. **Water Tank**: Reuse plastic bottles (free)
9. **Wait for Sales**: Online platforms often have deals
10. **Open Source**: Use free software and libraries

### 🔧 Alternative Components
- **Battery**: Use 4x 18650 cells with holder (₹200-300)
- **Chassis**: DIY from acrylic sheet (₹100-150)
- **Pump**: Repurpose from old fountain (free-₹100)
- **Wheels**: 3D print or use plastic bottle caps
- **Tank**: Use 500ml plastic bottle (free)

## Kit Options

Some vendors offer complete kits that may save money:

| Kit Type | Contents | Approx. Cost | Where to Buy |
|----------|----------|--------------|--------------|
| Arduino Starter Kit | Arduino + sensors + wires | ₹800-1,500 | Robu, Amazon |
| Motor Driver Kit | L298N + motors + wheels | ₹350-600 | ThinkRobotics |
| Sensor Kit | Multiple sensors including flame | ₹500-1,000 | ElectronicsComp |
| Robot Car Kit | Chassis + motors + driver | ₹600-1,200 | Robu, Amazon |

## Bulk Purchase Discounts

For institutions or group projects:

| Quantity | Typical Discount | Best For |
|----------|-----------------|----------|
| 5-10 units | 10-15% | Small group projects |
| 10-25 units | 15-25% | College workshops |
| 25+ units | 25-40% | Institution projects |

Contact wholesalers or distributors directly for best rates.

## Warranty & Quality Notes

| Component Type | Expected Lifespan | Warranty | Quality Check |
|----------------|------------------|----------|---------------|
| Arduino Board | 2-5 years | 3-6 months | Check for clones |
| Motor Driver | 1-3 years | 1-3 months | Verify heat sink |
| Motors | 500-1000 hrs | 1 month | Test RPM |
| Sensors | 1-2 years | 1-3 months | Test sensitivity |
| Battery | 300-500 cycles | 3-6 months | Check cells |

## Total Investment Summary

**For a single basic robot**: ₹2,500 - ₹3,500 (without tools)
**For complete setup with tools**: ₹4,000 - ₹6,000
**For premium build**: ₹6,000 - ₹9,000

**Ideal budget for student project**: ₹3,000 - ₹4,000

---

**Note**: Prices are approximate and may vary based on location, vendor, and market conditions. Always compare prices across multiple vendors and check for ongoing sales or discounts.

**Last Updated**: January 2026
